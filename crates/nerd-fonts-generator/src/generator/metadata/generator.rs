...
impl IconsMetadataGenerator {
    ...
    pub fn generate_keywords(icons: &[GlyphEntry<IconName>], registry: &IconMetadataRegistry) -> String {
    ...
    pub fn generate_categories(icons: &[GlyphEntry<IconName>], registry: &IconMetadataRegistry) -> String {
        let mut output = String::new();
        output.push_str("// @generated by build.rs — do not edit\n\n");
    ...
    /// Generates the aliases phf::Map content.
    pub fn generate_aliases(registry: &IconMetadataRegistry) -> String {
        let mut output = String::new();
        output.push_str("// @generated by build.rs — do not edit\n\n");
    ...
    pub fn run_with_registry(icons: &[GlyphEntry<IconName>], registry: &IconMetadataRegistry) -> Result<(), GenerateError> {
        let out_dir = std::env::var("OUT_DIR")?;
        let out_dir = PathBuf::from(out_dir);
        std::fs::create_dir_all(&out_dir)?;

        let keywords = Self::generate_keywords(icons, registry);
        std::fs::write(out_dir.join("keywords.rs"), keywords)?;

        let categories = Self::generate_categories(icons, registry);
        std::fs::write(out_dir.join("categories.rs"), categories)?;

        let aliases = Self::generate_aliases(registry);
...
impl NerdFontsGenerator for IconsMetadataGenerator {
    fn generate(icons: &[GlyphEntry<IconName>]) -> Result<String, GenerateError> {
        ...
        output.push_str(&Self::generate_keywords(icons, &empty_registry));
        output.push_str(&Self::generate_categories(icons, &empty_registry));
        output.push_str(&Self::generate_aliases(&empty_registry));
        Ok(output)
    }
