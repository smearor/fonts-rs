...
fn generate_metadata(
    ...
    keywords_code.push_str("pub static KEYWORDS: phf::Map<&'static str, &'static [&'static str]> = phf::phf_map! {\n");

    categories_code.push_str("// @generated by build.rs — do not edit\n\n");
    categories_code.push_str("/// Categories for each emoji glyph name.\n");
    categories_code.push_str("pub static CATEGORIES: phf::Map<&'static str, &'static str> = phf::phf_map! {\n");
    ...
    for entry in entries {
        ...
        if let Some(kws) = keyword_map.get(&cp) {
            if !kws.is_empty() {
                let kw_refs: Vec<String> = kws.iter().map(|k| format!("\"{k}\"")).collect();
        ...
        if let Some(cat) = category_map.get(&cp) {
            categories_code.push_str(&format!("    \"{glyph_name}\" => \"{cat}\",\n"));
        }
    }
