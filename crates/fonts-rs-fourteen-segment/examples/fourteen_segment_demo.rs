//! Interactive demo for fonts-rs-fourteen-segment.
//!
//! Demonstrates DSEG14 glyph resolution and GTK icon rendering.
//!
//! Run with:
//! ```sh
//! cargo run --example fourteen_segment_demo --features gtk,embed-fonts
//! ```
...
const APP_ID: &str = "io.smearor.fonts_rs.fourteen_segment_demo";
...
fn build_ui(app: &Application) {
    ...
    let window = ApplicationWindow::builder()
        .application(app)
        .title("fonts-rs-fourteen-segment Demo")
        .default_width(900)
        .default_height(700)
        .build();
    ...
    let title = Label::builder()
        .label(&format!("DSEG14 — Fourteen-Segment Display ({})", variant::GLYPH_PREFIX))
        .css_classes(["title-1"])
        .halign(Align::Center)
        .build();
    ...
    let hint = Label::builder()
        .label("Browse fourteen-segment display glyphs exported from the DSEG14 font.")
        .halign(Align::Center)
...
fn do_resolve(result_label: &Label, name: &str) {
    ...
    let glyph_name = FourteenSegmentName::new(name.to_string());
...
fn build_gallery_section() -> Frame {
    ...
    let desc = Label::builder()
        .label(&format!("DSEG14 glyphs ({}) rendered as GTK icons via GResource registration.", variant::GLYPH_PREFIX))
        .halign(Align::Start)
        .wrap(true)
        .build();
    section.append(&desc);
