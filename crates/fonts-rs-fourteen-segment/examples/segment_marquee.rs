//! Fourteen-Segment Marquee — a scrolling LED-style display using DSEG14 glyphs.
//!
//! Type text, watch it scroll across the fourteen-segment display like an old
//! digital readout. The display glows, the segments pulse, and you can see
//! which of the 24 style×weight variants is active.
//!
//! Run with:
//! ```sh
//! cargo run --example segment_marquee --features gtk,embed-fonts
//! ```
...
const APP_ID: &str = "io.smearor.fonts_rs.fourteen_segment_marquee";
...
/// All 24 variants: (feature_name, label, style_idx, weight_idx).
///
/// Style indices: 0=Classic, 1=Classic Mini, 2=Modern, 3=Modern Mini.
/// Weight indices: 0=Regular, 1=Bold, 2=Italic, 3=Bold Italic, 4=Light, 5=Light Italic.
const VARIANTS: &[(&str, &str, f32, f32)] = &[
    // Classic
    ("classic-regular", "Classic Regular", 0.0, 0.0),
    ("classic-bold", "Classic Bold", 0.0, 1.0),
    ("classic-italic", "Classic Italic", 0.0, 2.0),
    ("classic-bold-italic", "Classic Bold Italic", 0.0, 3.0),
    ("classic-light", "Classic Light", 0.0, 4.0),
    ("classic-light-italic", "Classic Light Italic", 0.0, 5.0),
    // Classic Mini
    ("classic-mini-regular", "Classic Mini Regular", 1.0, 0.0),
    ("classic-mini-bold", "Classic Mini Bold", 1.0, 1.0),
    ("classic-mini-italic", "Classic Mini Italic", 1.0, 2.0),
    ("classic-mini-bold-italic", "Classic Mini Bold Italic", 1.0, 3.0),
    ("classic-mini-light", "Classic Mini Light", 1.0, 4.0),
    ("classic-mini-light-italic", "Classic Mini Light Italic", 1.0, 5.0),
    // Modern
    ("modern-regular", "Modern Regular", 2.0, 0.0),
    ("modern-bold", "Modern Bold", 2.0, 1.0),
    ("modern-italic", "Modern Italic", 2.0, 2.0),
    ("modern-bold-italic", "Modern Bold Italic", 2.0, 3.0),
    ("modern-light", "Modern Light", 2.0, 4.0),
    ("modern-light-italic", "Modern Light Italic", 2.0, 5.0),
    // Modern Mini
    ("modern-mini-regular", "Modern Mini Regular", 3.0, 0.0),
    ("modern-mini-bold", "Modern Mini Bold", 3.0, 1.0),
    ("modern-mini-italic", "Modern Mini Italic", 3.0, 2.0),
    ("modern-mini-bold-italic", "Modern Mini Bold Italic", 3.0, 3.0),
    ("modern-mini-light", "Modern Mini Light", 3.0, 4.0),
    ("modern-mini-light-italic", "Modern Mini Light Italic", 3.0, 5.0),
];

/// Map an ASCII character to its DSEG14 glyph base name.
...
/// Load an SVG from a GResource, inject the given hex color as the fill color,
...
/// creating the texture. Handles both SVGs with `fill="currentColor"` and
/// SVGs with no fill attribute at all.
fn colorized_texture(resource_path: &str, hex_color: &str) -> Option<gtk4::gdk::Texture> {
    ...
    let colored = if svg.contains("currentColor") {
        svg.replace("currentColor", hex_color)
    } else {
        svg.replace("<path d=", &format!("<path fill=\"{hex_color}\" d="))
    };
...
fn build_ui(app: &Application) {
    ...
    let window = ApplicationWindow::builder()
        .application(app)
        .title("DSEG14 Fourteen-Segment Marquee")
        .default_width(800)
        .build();
    ...
    // --- Style slider (read-only) ---
    let active_idx = VARIANTS
        .iter()
        .position(|(feat, _, _, _)| variant::GLYPH_PREFIX == format!("dseg14-{feat}"))
        .unwrap_or(0);
    let (_active_feat, _active_name, active_style, active_weight) = VARIANTS[active_idx];

    let style_row = Box::builder().orientation(Orientation::Horizontal).spacing(8).build();

    let style_label = Label::builder().label("Style:").css_classes(["variant-label"]).build();
    style_row.append(&style_label);

    let style_scale = gtk4::Scale::with_range(Orientation::Horizontal, 0.0, 3.0, 1.0);
    style_scale.set_value(active_style as f64);
    style_scale.set_draw_value(false);
    style_scale.set_hexpand(true);
    style_scale.set_sensitive(false);
    style_scale.add_css_class("axis-scale");
    for (idx, name) in ["Classic", "Classic Mini", "Modern", "Modern Mini"].iter().enumerate() {
        let label = if idx as f32 == active_style {
            format!("<b><span foreground='#00ff66'>{name}</span></b>")
        } else {
            (*name).to_string()
        };
        style_scale.add_mark(idx as f64, gtk4::PositionType::Bottom, Some(&label));
    }
    style_row.append(&style_scale);
    controls.append(&style_row);
    ...
    let weight_scale = gtk4::Scale::with_range(Orientation::Horizontal, 0.0, 5.0, 1.0);
    weight_scale.set_value(active_weight as f64);
    ...
    for (idx, name) in ["Regular", "Bold", "Italic", "Bold Italic", "Light", "Light Italic"].iter().enumerate() {
        let label = if idx as f32 == active_weight {
            format!("<b><span foreground='#00ff66'>{name}</span></b>")
        } else {
            (*name).to_string()
        };
        weight_scale.add_mark(idx as f64, gtk4::PositionType::Bottom, Some(&label));
    }
    ...
    fg_btn.set_rgba(&gtk4::gdk::RGBA::new(0.0, 1.0, 0.4, 1.0));
    ...
    glow_btn.set_rgba(&gtk4::gdk::RGBA::new(0.2, 1.0, 0.6, 1.0));
    ...
    dim_btn.set_rgba(&gtk4::gdk::RGBA::new(0.0, 0.2, 0.1, 1.0));
    ...
    let state = Rc::new(RefCell::new(MarqueeState {
        ...
        fg_color: gtk4::gdk::RGBA::new(0.0, 1.0, 0.4, 1.0),
        glow_color: gtk4::gdk::RGBA::new(0.2, 1.0, 0.6, 1.0),
        dim_color: gtk4::gdk::RGBA::new(0.0, 0.2, 0.1, 1.0),
