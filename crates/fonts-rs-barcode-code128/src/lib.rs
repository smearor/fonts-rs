//! Libre Barcode Code 128 font integration for GTK 4.
//!
//! Provides glyph name resolution, GResource registration, and codepoint
//! lookup for the Libre Barcode Code 128 font family.
//!
//! ## Quick Start
//!
//! ```no_run
...
//!
//! ```no_run
//! use fonts_rs_barcode_code128::Code128Name;
//! use fonts_rs_barcode_code128::GlyphNameExt;
//!
//! let name = Code128Name::new("zero".to_string());
...
// Re-export key types
pub use definition::Code128;
pub use definition::Code128Definition;
pub use definition::Code128Name;

/// GResource prefix for this font family.
pub const GRESOURCE_PREFIX: &str = Code128Definition::GRESOURCE_PREFIX;

/// Extension trait adding codepoint lookup to `GlyphName<Code128>`.
...
impl GlyphNameExt for Code128Name {
    fn codepoint(&self) -> Option<CodePoint> {
        let key = self.as_ref();
...
/// Registers embedded Code 128 glyphs as a GResource.
///
...
/// Registers the Code 128 TTF font as a GResource.
///
...
mod tests {
    ...
    fn all_glyphs_is_not_empty() {
        ...
        assert!(count > 0, "Code 128 should export at least one glyph");
    }
