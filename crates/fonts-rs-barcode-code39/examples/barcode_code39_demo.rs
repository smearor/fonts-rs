//! Interactive demo for fonts-rs-barcode-code39.
//!
//! Demonstrates Libre Barcode Code 39 font rendering with a live input field.
//!
//! Run with:
//! ```sh
//! cargo run --example barcode_code39_demo --features gtk
//! ```
...
const APP_ID: &str = "io.smearor.fonts_rs.barcode_code39_demo";

const FONT_FAMILY: &str = "LibreBarcode39";
const FONT_PATH: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/resources/LibreBarcode39-Regular.ttf");

fn main() -> Result<glib::ExitCode> {
    gtk4::init().into_diagnostic()?;
    fonts_rs_barcode_code39::register_glyphs().into_diagnostic()?;
...
fn build_ui(app: &Application) {
    ...
    let window = ApplicationWindow::builder()
        .application(app)
        .title("Libre Barcode Code 39 Demo")
        .default_width(700)
        .default_height(300)
        .build();
    ...
    let hint = Label::builder()
        .label("Type text below. The barcode renders using the Code 39 font.\nCode 39 uses uppercase letters A–Z, digits 0–9, and special characters: - . $ / + % SPACE *")
        .halign(Align::Center)
        .wrap(true)
